...
namespace Emailit.Pages.Admin.Department
{
    ...
    public class EditModel : PageModel
    {
        ...
        public EditModel(ILogger<CreateModel> logger, IDepartmentRepository departmentRepository, IBranchOfficeRepository branchOfficeReporitory,
        ...
        public async Task<IActionResult> OnGetAsync(int? Id)
        {
            ...
            Models.Department depto;
            if (await CanDeactivateDepartments())
            {
                depto = await _departmentsData.GetEvenDeactivatedAsync(Id.GetValueOrDefault());

            }
            else
            {
                depto = await _departmentsData.GetAsync(Id.GetValueOrDefault());
            ...
            DepartmentId = Id.GetValueOrDefault();
            Name = depto.Name;
            BranchOfficeId = depto.BranchOfficeId.GetValueOrDefault();
            Active = depto.Active;

            if (depto.Manager != null)
            {

                ManagerIdCard = depto.Manager.IdCard;
            }
        ...
        public async Task<IActionResult> OnPostAsync(int id)
        {
            ...
            if (await CanDeactivateDepartments())
            {
                depto = await _departmentsData.GetEvenDeactivatedAsync(id);

            }
            else
            {
                depto = await _departmentsData.GetAsync(id);
            ...
            if ((!depto.Name.Equals(Conversions.ExtraSpaceRemover(Name), StringComparison.OrdinalIgnoreCase) || depto.BranchOfficeId != BranchOfficeId) &
                (await _departmentsData.GetAllEvenDeactivatedAsync()).Any(d => d.Name.Equals(Conversions.ExtraSpaceRemover(Name), StringComparison.OrdinalIgnoreCase) & d.BranchOfficeId == BranchOfficeId))
            {
            ...
            if (depto.ManagerId != user.UserId & (await _departmentsData.GetAllEvenDeactivatedAsync()).Any(d => d.ManagerId == user.UserId))
            {
            ...
            if (depto.BranchOffice == null)
            {
                depto.BranchOfficeId = BranchOfficeId;
            }
            else
            {
                depto.BranchOffice.BranchOfficeId = BranchOfficeId;
            }
            ...
            try
            {
                savedDepartment = await _departmentsData.UpdateAsync(depto);
            }
            catch (Exception ex)
            {
                _Logger.LogError($"Error modifying the department.");
            ...
            _Logger.LogInformation($"Department [ID: {depto.DepartmentId}] has been modified successfully.");
